...
% AKG-E Reframed: 2 Applications + VVUQ Backbone
...
\title{Agentic Knowledge Graphs with Simulation-in-the-Loop\\Validation for Construction and Transportation Engineering}
\author{}
\date{}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        \node[dombox] (con) {Construction\\[1pt]\tiny CV material takeoff,\\BOM reasoning, safety};
        ...
        \node[layerbox, below=0.25cm of onto] (sim) {\textbf{Simulation Integration Layer} --- FEA solvers, traffic microsimulators, thermal engines (agent-callable)};
    ...
    \caption{\textbf{AKG-E Framework with VVUQ.} Two engineering domains plug into shared ontology, simulation, and validation layers. Agents orchestrate retrieval, graph reasoning, and simulation calls. The VVUQ layer verifies outputs against physical laws, quantifies uncertainty, and enforces constraints. A feedback loop ensures iterative refinement until validation criteria are met.}
    \label{fig:framework}
    \vspace{-0.5em}
\end{figure}

The framework is demonstrated through two concrete applications that share the VVUQ backbone. In \textbf{construction engineering}, agents perform automated material takeoff using computer vision over architectural drawings, reason over bill-of-materials knowledge graphs, and validate structural designs through FEA simulation---with VVUQ ensuring load calculations satisfy building codes and quantifying safety margins. In \textbf{transportation engineering}, agents build and query digital twin models~\cite{Tao2019DigitalTwin} of traffic networks, optimize signal timing and routing, and validate proposals through microsimulation---with VVUQ verifying flow capacity constraints and quantifying prediction uncertainty under varying demand scenarios. Both applications exercise the same three-layer architecture; only the domain ontology and simulation interfaces differ.

This research investigates: (1)~how engineering domain ontologies can be represented as knowledge graphs enabling both LLM reasoning and structured constraint queries; (2)~how agents should orchestrate across retrieval, graph reasoning, and simulation to solve multi-step engineering problems; (3)~what VVUQ mechanisms ensure agent outputs satisfy physical laws with quantified uncertainty; and (4)~whether a shared framework achieves competitive accuracy against domain-specific systems while reducing development cost.

We will demonstrate AKG-E in construction (CV-based material takeoff, BOM validation, structural verification) and transportation (digital twin traffic optimization, network design). Agents invoke domain simulators, validate via VVUQ, and update digital twin state. Measurable targets include constraint violation reduction and decision accuracy vs.\ expert baselines. Expected outcomes include the open AKG-E framework, VVUQ-integrated engineering KG templates, and benchmarks demonstrating that \textbf{simulation-grounded, uncertainty-aware agentic AI} outperforms unstructured approaches for engineering decision-support.
