...
The funded year is structured as a 3-month aggressive build followed by 9 months of dissemination, robustness, and scaling.
\textit{Month~1:} generalize the LVL VVUQ pipeline (construction-ai/construction/design/vvuq-integration-plan.md) into the pluggable AKG-E framework; refactor KG curation and graph-RAG over the construction ontology; implement the propose-VVUQ-feedback agent loop on the construction pilot.
\textit{Month~2:} stand up the transportation digital twin per the existing VTSI extension proposal; begin cross-domain ablation runs across B0/B1/B2; build out the benchmark harness with per-cell logging and bootstrap analysis.
\textit{Month~3:} complete the 2-domain $\times$ 3-baseline $\times$ $n\geq5$-seed benchmark suite (construction + transportation); statistical analysis on all three metrics; first paper draft; scope the energy proof-of-concept (KG schema + PDE-solver interface) for Months~4--12 execution.
\textit{Months~4--12:} robustness experiments (model swaps, prompt perturbations, distribution shift), scaling studies on AWS, additional publications, and open-source release of the AKG-E framework, KG templates, and benchmark.
\textit{Deliverables:} (1)~the open-source AKG-E framework; (2)~domain-adaptable KG ontology templates and the typed FEA / digital-twin / PDE simulator-tool interface specification; (3)~the three-domain benchmark with ground-truth datasets; (4)~peer-reviewed publications targeting AI for engineering and agentic AI venues.
...
\textbf{Dr.~Kereshmeh Afsari (PI)} directs the ARCADE Lab at the Myers-Lawson School of Construction, with deep expertise in BIM, 3D visualization, and the construction-AI initiative that supplies AKG-E's primary domain testbed; she owns the ARCADE infrastructure (BIM/IFC datasets, simulation pipelines, AWS account) and the construction-industry partnerships that anchor the LVL pilot. \textbf{Jason Cusati (co-PI)} brings the agentic-KG research-progression substrate that supplies AKG-E's orchestration layer, the VTTI Virginia Transportation Safety Index calculator (and its digital-twin extension proposal) that anchors the transportation domain, a published software-engineering evaluation paper that surfaced the verification weaknesses AKG-E sets out to close, and direct VVUQ training under Dr.~Christopher Roy's graduate coursework (Oberkampf--Roy formulation~\cite{Oberkampf2010VVUQ}).
