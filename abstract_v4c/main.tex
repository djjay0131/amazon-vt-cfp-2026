...
% AKG-E Reframed: Construction-only + VVUQ as core contribution
...
\title{Agentic Knowledge Graphs with Simulation-in-the-Loop\\VVUQ for Construction Engineering}
\author{}
\date{}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        dombox/.style={rectangle, rounded corners=3pt, draw=gray!50, fill=orange!6, minimum width=5.5cm, minimum height=1.1cm, align=center, font=\tiny},
        ...
        % Domain application (top) - single centered construction domain
        \node[dombox] (con) {Construction Engineering\\[1pt]\tiny CV material takeoff, BOM reasoning,\\structural verification via FEA};

        % Domain ontology layer
        \node[layerbox, below=0.4cm of con] (onto) {\textbf{Domain Ontology Layer} --- Engineering KGs: entities, constraints, physical laws, design rules};

        % Simulation integration layer
        \node[layerbox, below=0.25cm of onto] (sim) {\textbf{Simulation Integration Layer} --- FEA solvers (agent-callable)};
    ...
    \caption{\textbf{AKG-E Framework with VVUQ.} Construction engineering plugs into shared ontology, simulation, and validation layers. Agents orchestrate retrieval, graph reasoning, and FEA simulation calls. The VVUQ layer verifies outputs, quantifies uncertainty, and enforces building code constraints. A feedback loop ensures iterative refinement until validation criteria are met.}
    \label{fig:framework}
    \vspace{-0.5em}
\end{figure}

We demonstrate AKG-E in \textbf{construction engineering}, where agents parse IFC/BIM drawings to extract structural elements (columns, beams, connections) and populate a construction knowledge graph encoding materials, load specifications, and code requirements. Agents then perform automated material takeoff using computer vision, reason over bill-of-materials KGs grounded in BIM/IFC standards, call FEA solvers for structural analysis, and build and query a construction digital twin for real-time project state tracking and schedule-aware design iteration. VVUQ ensures load calculations satisfy building codes, quantifies safety margins under material property uncertainty via Monte Carlo propagation, and flags outputs exceeding uncertainty thresholds for human review before deployment. Construction is an ideal testbed because it combines structured ontological knowledge (BIM/IFC), physics-based simulation (FEA), and multi-level regulatory constraint satisfaction---exercising all three AKG-E layers on a single, high-stakes domain.

This research tests two hypotheses: \textbf{H1:}~Agents grounded in engineering KGs with simulation-in-the-loop VVUQ produce significantly fewer physically infeasible outputs than RAG-only or prompt-only LLM baselines on construction tasks (material takeoff accuracy, BOM validation, structural verification; target: $>$50\% constraint violation reduction). \textbf{H2:}~VVUQ-aware agent pipelines yield calibrated uncertainty estimates for engineering quantities (coverage within 10\% of nominal), enabling principled confidence thresholds for autonomous versus human-reviewed decisions.

Experiments compare prompt-only LLMs, RAG-augmented methods, and full AKG-E on construction material takeoff, BOM validation, structural verification, and construction planning tasks, with expert-annotated ground truth from BIM models. Expected outcomes include the open AKG-E framework, VVUQ-integrated KG templates, and benchmarks demonstrating that \textbf{simulation-grounded agentic AI} outperforms unstructured approaches for engineering decision-support.
