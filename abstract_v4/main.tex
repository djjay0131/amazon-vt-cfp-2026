...
The key insight is that engineering problems, despite domain differences, share structural commonalities that enable a general-purpose agentic framework. A construction scheduling agent reasoning over bill-of-materials constraints and a traffic optimization agent reasoning over network flow equations both require: structured knowledge representation encoding domain entities and relationships; agent-callable interfaces to simulation tools; constraint checking that prevents physically impossible or regulation-violating outputs; and provenance tracking linking recommendations to specific engineering knowledge and simulation results~\cite{OpenTelemetryGenAI2024}. AKG-E abstracts these shared requirements into reusable layers while domain-specific knowledge graphs and simulators remain pluggable. This design yields a \textit{generalizable method}---constraint-grounded agentic reasoning with simulation-in-the-loop validation---not merely a domain application.
...
We will implement AKG-E with domain instantiations across \textbf{construction planning} (BOM reasoning, schedule risk prediction, safety compliance), \textbf{transportation systems} (traffic flow optimization, network design), and \textbf{thermal engineering} (heat transfer analysis, HVAC sizing). Measurable targets include constraint violation reduction rates and engineering decision accuracy vs.\ expert baselines. Experiments compare prompt-only LLM approaches, RAG-augmented methods, and the full AKG-E framework. Expected outcomes include the AKG-E open framework, engineering KG templates, and benchmarks demonstrating that \textbf{structured, simulation-grounded agentic AI} substantially outperforms unstructured approaches for engineering problem-solving.
